...
class CalcRepository:
    ...
    def setRepoData(self, searchString, category="", extension="", math=False,
                    game=False, searchFiles=False):
        """Call this function with all the settings to use for future operations
        on a repository, must be called FIRST"""
    ...
    def setOutputObject(self, newOutput=output.CalcpkgOutput(True, True)):
        """Set an object where all output from calcpkg will be redirected to for
        this repository"""
    ...
    def searchIndex(self, printData=True):
        ...
        self.data = self.index.search(
            self.searchString, self.category, self.math, self.game,
            self.searchFiles, self.extension
        )
        self.output.printData = backupValue
        return self.data

    def countIndex(self):
        """A wrapper for the count function in calcrepo.index; count using
        specified parameters"""
        self.data = self.index.count(
            self.searchString, self.category, self.math, self.game,
            self.searchFiles, self.extension
        )
    ...
    def getFileInfos(self):
        ...
        for datum in data:
            try:
                ...
                self.printd(
                    "Error: the info command is not supported for " + self.name + "."
                )
                return []
        return fileInfos
    ...
    def downloadFiles(self, prompt=True, extract=False):
        ...
        if prompt:
            if sys.version_info.major == 3:
                confirm = input("Download files [Y/N]? ")
            else:
        ...
        for datum in data:
            ...
            if extract:
                ...
                if extractType != "":
                    self.printd("Extracting file " + dowName +
                                ", creating directory for extracted files")
                    ...
                    os.remove(dowName)
                    self.printd("The archive file " +
                                dowName + " has been deleted!\n")
    ...
    def openIndex(self, filename, description):
        """Attempt to delete and recreate an index, returns open file object
        or None."""
        try:
            os.remove(filename)
        ...
        try:
            ...
            self.printd("Error: Unable to create file " +
                        filename + " in current folder. Quitting.")
            return None
        return files
