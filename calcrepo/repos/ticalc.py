...
class TicalcRepository(repo.CalcRepository):
    ...
    def getFileInfo(self, fileUrl, fileName):
        ...
        fileInfo.fileDate = self.getBaseFileData(
            infoText, "File Date and Time", 47, 2)
        fileInfo.documentation = self.getBaseFileData(
            infoText, "Documentation&nbsp;Included?")
    ...
    def getFileAuthor(self, fileInfo):
        """Function to get the file's author for file info, note that we are
        pretending that multiple authors do not exist here"""
