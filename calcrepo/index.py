...
class Index:
    """Index object created for each repository: note, we're assuming that local
    copies of the indexes will use the same formats"""
    ...
    def count(self, searchString, category="", math=False, game=False,
              searchFiles=False, extension=""):
        """Counts the number of ticalc.org files containing some search term,
        doesn't return them"""
        fileData = {}
        nameData = {}
        ...
        # Search the index
        if searchFiles:
            fileData = self.searchNamesIndex(
                self.fileIndex, fileData, searchString, category, math, game,
                extension
            )
        else:
        ...
        if searchFiles:
            nameData, fileData = self.searchFilesIndex(
                fileData, nameData, self.nameIndex, searchString)
        else:
            fileData, nameData = self.searchFilesIndex(
                nameData, fileData, self.fileIndex, searchString, category, math,
                game, extension
            )
        ...
        if fileData is None or nameData is None:
            self.repo.printd(
                "Error: failed to load one or more of the index files for this " +
                "repo. Exiting."
            )
        ...
        self.repo.printd("Search for '" + searchString + "' returned " +
                         str(count) + " result(s) in " + self.repo.name)
        return count

    def searchHierarchy(self, fparent):
        """Core function to search directory structure for child files and folders
        of a parent"""
        returnData = []
        ...
        for fldr in dirFile:
            ...
            if fparent in fldr and parentslashes + 1 == fldr.count('/'):
                returnData.append(
                    [fldr, fldr[fldr.rfind('/', 1, -1):-1], ResultType.FOLDER])
                foldercount += 1
    ...
    def search(self, searchString, category="", math=False, game=False,
               searchFiles=False, extension=""):
        ...
        if len(data) != 0:
            ...
            self.repo.printd(structureOutput("File Category:",
                                             "File Name:", False, False, space))
        ...
        for datum in data:
            try:
                self.repo.printd(structureOutput(
                    datum[0], datum[1], searchFiles, True, space))
    ...
    def searchFilesIndex(self, nameData, fileData, fileIndex, searchString,
                         category="", math=False, game=False, extension=""):
        ...
        for line in fileFile:
            ...
            try:
                if nameData[count] is not None:
                    ...
                    # Both game and math
                    if (game and math):
                        if ("/games/" in line or "/math/" in line or
                                "/science" in line):
    ...
    def searchNamesIndex(self, nameIndex, nameData, searchString, category="",
                         math=False, game=False, extension="",
                         searchFiles=False):
...
def structureOutput(fileUrl, fileName, searchFiles, format=True, space=40):
    ...
    if format:
        ...
        for splitUrl in splitUrls:
            ...
            if splitUrl != "" and ("." not in splitUrl) and (
                    splitUrl != "pub" and splitUrl != "files"):
                fileUrl += splitUrl + '/'
            elif "." in splitUrl:
